...
build:
	...
	@PIP_OPTS="--ignore-installed urllib3" bash-tools/python_pip_install_if_absent.sh --ignore-installed urllib3 requirements.txt
...
unittest:
	...
	`bash-tools/python_find_library_executable.sh nose2 nosetests-2.7 nosetests`
